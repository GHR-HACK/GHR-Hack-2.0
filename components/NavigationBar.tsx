...
export default function NavigationBar() {
  ...
  return (
    <nav
      ref={navRef}
      className="fixed top-0 left-0 right-0 z-50 bg-white text-black backdrop-blur-lg border-b border-purple-500/20 transition-all duration-300"
    >
      <div className="w-full">
        <div className="flex items-center justify-between h-20 px-4 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className="h-12 w-[2px] rounded-full bg-black/40"
            />
            <button
              ref={rightLogoRef}
              onClick={() => scrollToSection('#home')}
          ...
          <div className="hidden lg:flex items-center gap-4 xl:gap-6">
            {navItems.map((item, index) => (
              <button
                ...
                className="relative font-red-hat-display font-semibold text-base lg:text-lg px-2 py-1 group hover:text-primary-orange transition-colors duration-300 text-black"
              >
                {item.name}
          ...
          <div className="lg:hidden flex items-center">
            ...
            >
              <div className="relative w-6 h-5">
                <span className="hamburger-top absolute top-0 left-0 w-6 h-0.5 rounded-full transition-colors duration-300 bg-black"></span>
                <span className="hamburger-middle absolute top-1/2 left-0 w-6 h-0.5 rounded-full -translate-y-1/2 transition-colors duration-300 bg-black"></span>
                <span className="hamburger-bottom absolute bottom-0 left-0 w-6 h-0.5 rounded-full transition-colors duration-300 bg-black"></span>
              </div>
            </button>
          </div>
        </div>
      ...
      >
        <div className="px-6 pb-4 border-t transition-all duration-300 bg-white border-black/20">
          <div className="pt-4 space-y-2">
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className="block w-full text-left px-4 py-3 font-red-hat-display font-semibold rounded-lg hover:bg-gradient-to-r hover:from-primary-orange/10 hover:to-primary-purple/10 hover:text-primary-orange transition-all duration-300 border border-transparent hover:border-primary-orange/20 text-black"
              >
                {item.name}
              </button>
            ))}
