...
export default function Sponsors() {
  ...
  const renderSponsorTier = (tier: string, sponsorList: any[], tierClass: string) => (
    <div className={`sponsor-tier mb-10 ${tierClass}`}>
      <Title level={4} variant="default" size="md" className={`text-center mb-8 !text-[#ff5100]`}>
          ...
          <div key={index} className="sponsor-logo-container h-24 flex items-center justify-center hover:scale-110 transition-transform duration-300">
            {sponsor.logo ? (
              <img
                src={sponsor.logo}
                alt={sponsor.name}
                className="sponsor-logo max-h-36 max-w-32 object-contain"
              />
            ) : (
  ...
  return (
    <section
      id="sponsors"
      ref={sectionRef}
      className="py-16 md:py-20 bg-primary-purple/10"
    >
