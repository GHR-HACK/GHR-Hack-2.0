...
export async function POST(request: NextRequest) {
  try {
    ...
    const updateResult = await teamsCollection.findOneAndUpdate(
      {
        _id: teamId,
        // Either no PS selected yet, or already selected this PS
        $or: [
    ...
    if (!updateResult.value) {
      ...
      return NextResponse.json(
        {
          error: 'PS selection failed. Your team may have already selected a different PS or another team claimed this PS.',
        },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      {
        ...
        team: {
          ...
          name: updateResult.value.team_name,
          selected_ps: updateResult.value.selected_ps.toString(),
          selected_at: updateResult.value.selected_at,
        },
