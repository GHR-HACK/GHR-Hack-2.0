...
export default function SelectPSPage() {
  ...
  const domains = ['All', ...Array.from(new Set(problemStatements.map(ps => ps.domain)))];

  // Filter PS by selected domain
  const filteredPS = selectedDomain === 'All' 
    ? problemStatements 
    ...
    <div className="min-h-screen bg-white text-black">
      <div className="px-4 pb-16 pt-28">
        <Container size="lg">
                ...
                >
                  ...
                  {domain !== 'All' && (
                    <span className={`ml-2 ${selectedDomain === domain ? 'text-white/80' : 'text-black/50'}`}>
                      ({problemStatements.filter(ps => ps.domain === domain).length})
                    </span>
                  )}
                  {domain === 'All' && (
                    <span className={`ml-2 ${selectedDomain === domain ? 'text-white/80' : 'text-black/50'}`}>
          ...
          {problemStatements.length > 0 && filteredPS.length === 0 && (
            <div className="text-center py-12">
              <Title level={3} variant="primary" className="mb-2">
