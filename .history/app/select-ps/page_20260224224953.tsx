...
export default function SelectPSPage() {
  ...
  const { data: psTeamCounts = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (problemStatements.length > 0) {
        // For now, set default counts to 0
        // You can add an API endpoint to get actual counts if needed
  ...
  const selectMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['problemStatements'] });
      setTimeout(() => {
        router.push('/shortlisted-teams');
      }, 1500);
    },
    onError: (error: any) => {
